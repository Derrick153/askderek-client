...
const ImagePreviews = ({ images }: ImagePreviewsProps) => {
  ...
  const imgs = images?.length > 0 
    ? images.map((url: string) => 
        url.includes('unsplash.com') && !url.includes('?') 
          ? `${url}?w=1200&q=80` 
          : url
      )
    ...
    <div className="relative h-[400px] sm:h-[550px] w-full overflow-hidden bg-slate-950 rounded-2xl border border-white/5 shadow-2xl">
      {imgs.map((image, index) => (
        ...
        >
          <img
            ...
            className="object-cover w-full h-full transition-transform duration-[2000ms] hover:scale-110"
          />
        </div>
      ))}
      ...
      <div className="absolute top-6 left-6 flex items-center gap-2 bg-slate-950/60 backdrop-blur-md border border-white/10 px-3 py-1.5 rounded-lg select-none">
        ...
        <span className="text-[10px] font-black text-white uppercase tracking-widest">
          Verified <span className="text-orange-500 italic">Property</span>
        </span>
      </div>
      ...
      {imgs.length > 1 && (
        <>
          ...
          <div className="absolute inset-x-4 top-1/2 -translate-y-1/2 flex justify-between items-center z-10 pointer-events-none">
            <button
              onClick={prev}
              className="pointer-events-auto w-12 h-12 bg-white/10 hover:bg-orange-500 backdrop-blur-md border border-white/20 rounded-xl flex items-center justify-center transition-all active:scale-90 group"
            >
