...
export default function OfficePage() {
  ...
  const { data: propsRaw, isLoading } = useGetPropertiesQuery({ propertyType: "OFFICE" } as any);
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white py-16 px-4">
        <div className="max-w-5xl mx-auto text-center">
          <div className="w-14 h-14 bg-slate-500 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Briefcase className="w-7 h-7 text-white" />
          </div>
          <h1 className="text-3xl font-black mb-3">Office Spaces in Ghana</h1>
          <p className="text-slate-400 text-sm mb-8">Professional office spaces for your business across Ghana</p>
          <div className="relative max-w-lg mx-auto">
            ...
            <input type="text" placeholder="Search by name or location..." value={search} onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-12 pr-4 py-3.5 text-sm bg-white text-gray-900 rounded-2xl focus:outline-none focus:ring-2 focus:ring-slate-500/40" />
          </div>
        </div>
      </div>
      <div className="max-w-5xl mx-auto px-4 py-10">
        <p className="text-sm text-gray-500 mb-6"><span className="font-bold text-gray-900">{filtered.length}</span> office spaces found</p>
        {isLoading ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-20 text-center">
            ...
            <p className="text-sm text-gray-500">{search ? "Try a different search term." : "No office listings yet."}</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((p: any) => (
              <div key={p.id} onClick={() => router.push(`/search/${p.id}`)}
                ...
                <div className="relative h-48 bg-gray-100">
                  {p.photoUrls?.[0] ? (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center"><Briefcase className="w-10 h-10 text-gray-300" /></div>
                  )}
                  <div className="absolute top-3 left-3"><ListingTypeBadge type="OFFICE" size="sm" /></div>
                </div>
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-lg font-black text-slate-600">{formatGHS(p.pricePerMonth)}</p>
                      <p className="text-xs text-gray-400">per month</p>
                    </div>
                    <div className="w-8 h-8 bg-slate-50 rounded-xl flex items-center justify-center group-hover:bg-slate-100 transition-colors">
