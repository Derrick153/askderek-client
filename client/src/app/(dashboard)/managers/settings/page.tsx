...
import { Settings, Save, User, Mail, Phone, Shield, LogOut } from "lucide-react";
...
export default function ManagerSettingsPage() {
  ...
  const [updateManager, { isLoading }] = useUpdateManagerSettingsMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user?.id) return;
    try {
      await updateManager({ userId: user.id, ...form }).unwrap();
      setSaved(true);
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-sm">Manage your manager profile</p>
          </div>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden">
          <div className="bg-gradient-to-r from-orange-600/10 to-red-600/5 border-b border-zinc-800 px-6 py-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-black text-white text-lg">{form.name || "Manager"}</p>
                <p className="text-zinc-500 text-sm">{form.email}</p>
                <span className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 bg-orange-600/15 border border-orange-600/30 rounded-md text-xs font-bold text-orange-400">
                  <Shield className="w-3 h-3" />Property Manager
                </span>
              </div>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <Input value={form.email} onChange={(e: any) => setForm({ ...form, email: e.target.value })}
                  type="email" className="pl-10" placeholder="your@email.com" />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <Input value={form.phoneNumber} onChange={(e: any) => setForm({ ...form, phoneNumber: e.target.value })}
                  className="pl-10" placeholder="024 XXX XXXX" />
              </div>
              <p className="text-xs text-zinc-600 mt-1.5">Used for WhatsApp contact with tenants</p>
            </div>

            <button
              type="submit"
              disabled={isLoading}
              className={`w-full py-3.5 rounded-xl font-black text-sm tracking-wide flex items-center justify-center gap-2 transition-all ${
                saved
